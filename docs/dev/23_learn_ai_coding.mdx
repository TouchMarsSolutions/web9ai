...
:::tip car mold
```md
You've played many games that have cars, right? Remember how you can customize the car's color, model, and speed?
Let's build a Car Class in Java, which has:
- speed
- color
- model
and the car can do some actions right? such as:
- start
- accelerate
- brake
```
...
classDiagram
    ...
    note for Car "speed is initialized to 0min the constructor"
```
